...
#=====================================================================FUNCIONES PARA GESTIONAR ERRORES==================================================================
#validacion de ruta--------------------------------------------------------------------------------
validar_ruta() {
	if [ ! -d "$1" ]; then
...
crear_verificado() {
	...
	nombre=$(basename "$1")
	if [ -d "$1" ]; then
		echo -e "${amarillo}Ya existe el directorio '$1', no podemos crearlo y ser redundantes${reset}
	else
		mkdir -p "$1"
		echo -e "${verde}Carpeta creada${reset}"
	fi
}
...
menu() {
	...
	while true; do
		...
		case $opcion in
			1) read -p "Ruta a listar: " r; valdiar_ruta "$r" && ls -l "$r";;
			2) read -p "Ruta a crear: " r; crear_si_no_existe "$r" && echo -e "${verde}Creado correctamente${reset}";;
			3) read -p "Tipo de plantilla (software/estudio/personal:<nombre>): " t; read -p "Ruta destino: " r; validar_ruta "$r" && crear_plantilla_pers "$t" "$r";;
			4) read -p "Ruta a borrar: " r; validar_ruta "$r" && mv "$r" "$HOME/.gestor_papelera/" && echo -e "${amarillo}Movido a la papelera${reset}";;
			5) read -p "Ruta a respaldar: " r; validar_ruta "$r" && tar -czf "backup_$(date +%F_%H-%M-%S).tar.gz" "$r" && echo -e "${verde}Backup exitoso${reset}";;
			6) read -p "Archivo .tar.gz a recuperar :" f; tar -xzf "$f" && echo -e "${verde}Restaurado exitosamente${reset}";;
			7) read -p "Nombre de la ruta actual: " a; read -p "Nuevo nombre: " n; validar_ruta "$a" && mv "$a" "$n" && echo -e "${verde}Ruta renombrada${reset}";;
			8) read -p "Nombre a buscar: " n; read -p "Ruta base: " r; validar_ruta "$r" && find "$r" -iname "$n";;
			9) read -p "Ruta base: " r; validar_ruta "$r" && find "$r" -type d -empty -delete && echo -e "${verde}Carpetas vacías eliminadas${reset}";;
...
case "$1" in
	...
	--crear) crear_si_no_existe "$2" && echo -e "${verde}Creado correctamente${reset}" ;;
	--plantilla) crear_plantilla "$2" "$3" ;;
	--crear-plantilla) crear_plantilla_pers "$2" ;;
	--borrar) validar_ruta "$2" && mv "$2" "$HOME/.gestor_papelera/" && echo -e "${amarillo}Movido a la papelera${reset}" ;;
	--papelera) listar_papelera ;;
	--restaurar-papelera) restaurar_papelera "$2" ;;
	--vaciar-papelera) vaciar_papelera ;;
	--backup) validar_ruta "$2" && tar -czf "backup_$(date +%F_%H-%M-%S).tar.gz" "$2" && echo -e "${verde}Backup exitoso${reset}" ;;
	--restaurar) tar -xzf "$2" && echo -e "${verde}Backup recuperado${reset}" ;;
	--renombrar) validar_ruta "$2" && mv "$2" "$3" && echo -e "${verde}Ruta renombrada${reset}" ;;
	--buscar) validar_ruta "$3" && find "$3" -iname "$2" ;;
	--limpiar) validar_ruta "$2" && find "$2" -type d -empty -delete && echo -e "${verde}Carpetas vacias eliminadas${reset}" ;;
